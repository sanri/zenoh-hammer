...
impl Default for AppHexViewer {
    fn default() -> Self {
        let selected_page = Page::Bool;
        let (base_info, data, arc_data) = example_data(selected_page);
            ...
            ComboBox::new("show_sample_viewer_combobox", "")
                ...
                .show_ui(ui, |ui| {
                    for selected in Page::iter() {
                        ...
                        if r.changed() {
                            let (base_info, data, arc_data) = example_data(selected);
                            self.viewer = SampleViewer::new(base_info, data, arc_data);
                        }
                    }
                });
        });
...
fn example_data(page: Page) -> (BaseInfo, ZBytes, Arc<Vec<u8>>) {
    ...
    let z_bytes = match page {
        ...
        Page::Bool => {
            base_info.encoding = Encoding::ZENOH_BOOL;
            ZBytes::from(true)
        }
        Page::Int8 => {
            base_info.encoding = Encoding::ZENOH_INT8;
            ZBytes::from(-1i8)
        }
        Page::Int16 => {
            base_info.encoding = Encoding::ZENOH_INT16;
            ZBytes::from(-2i16)
        }
        Page::Int32 => {
            base_info.encoding = Encoding::ZENOH_INT32;
            ZBytes::from(-3i32)
        }
        Page::Int64 => {
            base_info.encoding = Encoding::ZENOH_INT64;
            ZBytes::from(-4i64)
        }
        Page::Int128 => {
            base_info.encoding = Encoding::ZENOH_INT128;
            ZBytes::from(-5i128)
        }
        Page::Uint8 => {
            base_info.encoding = Encoding::ZENOH_UINT8;
            ZBytes::from(1u8)
        }
        Page::Uint16 => {
            base_info.encoding = Encoding::ZENOH_UINT16;
            ZBytes::from(2u16)
        }
        Page::Uint32 => {
            base_info.encoding = Encoding::ZENOH_UINT32;
            ZBytes::from(3u32)
        }
        Page::Uint64 => {
            base_info.encoding = Encoding::ZENOH_UINT64;
            ZBytes::from(4u64)
        }
        Page::Uint128 => {
            base_info.encoding = Encoding::ZENOH_UINT128;
            ZBytes::from(5u128)
        }
        Page::Float32 => {
            base_info.encoding = Encoding::ZENOH_FLOAT32;
            ZBytes::from(1.0f32)
        }
        Page::Float64 => {
            base_info.encoding = Encoding::ZENOH_FLOAT64;
            ZBytes::from(2.0f64)
        }
        Page::String => {
        ...
        Page::Error => {
            base_info.encoding = Encoding::ZENOH_ERROR;
            ZBytes::from("invalid parameter")
        }
        Page::Json => {
